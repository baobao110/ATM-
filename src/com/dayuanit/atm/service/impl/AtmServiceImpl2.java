...
public class AtmServiceImpl2 implements AtmService {
	...
	public void deposit(String amount, String cardNum, String password) {
		...
		try {
			...
			int rows = cardDao.modifyBalance(bankCard.getCardNum(), bankCard.getBalance(), bankCard.getVersion());
			if (1 != rows) {
	...
	public void transfer(String amount, String inCardNum, String outCardNum, String password) {
		...
		try {
			...
			BankCard outCard = cardDao.getBankCard(outCardNum);
			...
			int rows = cardDao.modifyBalance(outCard.getCardNum(), outCard.getBalance(), outCard.getVersion());
			if (1 != rows) {
			...
			//转入账户操作
			BankCard inCard = cardDao.getBankCard(inCardNum);
			if (null == inCard) {
			...
			rows = cardDao.modifyBalance(inCard.getCardNum(), inCard.getBalance(), inCard.getVersion());
			if (1 != rows) {
				throw new BizException("转账失败");
			}
